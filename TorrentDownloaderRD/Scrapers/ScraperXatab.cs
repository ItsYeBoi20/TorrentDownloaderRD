...
{
    internal class ScraperXatab
    {
        private const string SearchUrl = "https://hydralinks.cloud/sources/xatab.json";
        ...
        public static async Task ScrapeTorrentsAsync(string searchText, Action<TorrentInfo> updateCallback)
        {
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    foreach (var result in responseJson["downloads"])
                    {
                        ...
                        if (MatchesSearchTerms(title, searchTerms))
                        {
                            ...
                            Main.magnetLinksXatab[name] = magnetLink;
            ...
            {
                // Console.WriteLine($"Error scraping Xatab Repacks: {ex.Message}");
            }
        }
